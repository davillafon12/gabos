...
function llamarFacturas(){
	if(validarFechas()){		
		$.ajax({
			url : location.protocol+'//'+document.domain+'/consulta/getRecibosFiltrados',
			type: "POST",	
...
function cargarEncabezado(consecutivo){
	$.ajax({
		url : location.protocol+'//'+document.domain+"/consulta/getRecibo",
		type: "POST",
...
function imprimir(){
	...
	if(tipoImpresion==='t'){
		...
		//Impresion carta
		window.open(location.protocol+'//'+document.domain+'/impresion?t='+token+'&d=r&n='+consecutivoActual+'&s='+sucursal+'&i='+tipoImpresion,'Impresion de Facturas','width='+anchoImpresion+',height='+alturaImpresion+',resizable=no,toolbar=no,location=no,menubar=no');
	}
}
