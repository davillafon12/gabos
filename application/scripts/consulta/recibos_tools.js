...
function llamarFacturas(){
	if(validarFechas()){		
		$.ajax({
			url : location.protocol+'//'+document.domain+(location.port ? ':'+location.port: '')+'/consulta/getRecibosFiltrados',
			type: "POST",	
...
function manejarErrores(error){
	switch(error){
		case '1':
			notyMsg('No se pudo realizar la búsqueda, por favor contacte al administrador', 'error');
		break;
		case '2':
			notyMsg('No URL tiene un formato inválido, por favor contacte al administrador', 'error');
		break;
		case '3':
			notyMsg('No hay recibos con los filtros ingresados', 'warning');
...
function cargarEncabezado(consecutivo){
	$.ajax({
		url : location.protocol+'//'+document.domain+(location.port ? ':'+location.port: '')+"/consulta/getRecibo",
		type: "POST",
		data: {'recibo':consecutivo},		
		success: function(data, textStatus, jqXHR)
		{			
			try{
				facturaHEAD = $.parseJSON('[' + data.trim() + ']');
				if(facturaHEAD[0].status==="error"){
					notyMsg("Error al cargar el recibo, contacte al administrador. ERROR E"+facturaHEAD[0].error, "error");					
				}else if(facturaHEAD[0].status==="success"){
					setEncabezadoFactura(facturaHEAD[0].recibo);
...
function setEncabezadoFactura(facturaHEAD){	
	$("#cedula_cliente").html(facturaHEAD[0].cliente_cedula);
	$("#nombre_cliente").html(facturaHEAD[0].cliente_nombre);
	
	$("#consecutivo_factura").html(facturaHEAD[0].factura);
	$("#fecha_factura").html(facturaHEAD[0].fecha_expedicion);
	$("#monto_factura").html(parseFloat(facturaHEAD[0].Saldo_inicial).format(2, 3, '.', ','));
	
	$("#fecha_recibo").html(facturaHEAD[0].fecha_recibo);
...
function imprimir(){
	...
	if(tipoImpresion==='t'){
		//Impresion termica
		window.open(location.protocol+'//'+document.domain+(location.port ? ':'+location.port: '')+'/impresion/termica?t='+token+'&d=r&n='+consecutivoActual+'&s='+sucursal+'&i='+tipoImpresion+'&server='+document.domain+'&protocol='+location.protocol,'Impresion de Facturas','width='+anchoImpresion+',height='+alturaImpresion+',resizable=no,toolbar=no,location=no,menubar=no');
