...
$(window).load(function(){
	$(".contenedor-descripcion").dblclick(cambiarDescripcion);
});
...
function agregarFila(index){
	...
	cell3.innerHTML = "<input id='cantidad_articulo_"+siguienteFila+"' class='cantidad_articulo' autocomplete='off' name='cantidad_articulo' type='number' min='1' onchange='cambiarCantidad(this.id, event, this.value);' onkeyup='cambiarCantidad(this.id, event, this.value);'>"
					 +"<input id='cantidad_articulo_anterior_"+siguienteFila+"' type='hidden' value='-1'>";
	cell2.className = "contenedor-descripcion";
	cell2.innerHTML = "<input type='text' class='art-descripcion-guardado' id='desc_final_"+siguienteFila+"' style='display:none;' onkeyup='actualizarDescripcion(event)'/>"
	...
	$(".contenedor-descripcion").unbind("dblclick");
	$(".contenedor-descripcion").dblclick(cambiarDescripcion);
}
...
function setDatosArticulo(articuloARRAY, id_input, num_row, cantidadArticulos){
	...
	*/
	...
	else
	{
		...
		document.getElementById("producto_retencion_"+num_row).value=articuloARRAY[11];
...
function cambiarDescripcion(e){
	var container = $(e.target).prop("nodeName") === "DIV" ? $(e.target).parent() : $(e.target);
	const value = $(container).find(".articulo_specs").html();
	if(value.trim() !== ""){
		$(container).find(".articulo_specs").hide();
		$(container).find(".art-descripcion-guardado").show();
	}
}

function actualizarDescripcion(e){
	if(e!=null){
		const descr = $(e.target).val();
		if (e.keyCode == 13 && descr.trim() !== "") {
			var container = $(e.target).parent();
			$(container).find(".articulo_specs").html($(e.target).val());
			$(container).find(".articulo_specs").show();
			$(container).find(".art-descripcion-guardado").hide();
		}
	}
}