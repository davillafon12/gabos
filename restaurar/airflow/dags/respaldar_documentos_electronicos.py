...
sincronizar_archivos = SSHOperator(ssh_conn_id='gabo_app_ssh',
    ...
    conn_timeout=1200,
    cmd_timeout=1200,
    command="sshpass -p '" + docker_host_user_password + "' rsync -rl -e 'ssh -p " + docker_host_port + " -o StrictHostKeyChecking=no' '{{var.value.ruta_fuente_documentos_electronicos}}'  " + docker_host_user + "@" + docker_host_url + ":'{{var.value.ruta_respaldo_documentos_electronicos}}'",
    dag=dag)
