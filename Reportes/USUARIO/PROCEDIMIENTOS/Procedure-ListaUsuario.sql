DROP PROCEDURE `gabo_fe`.`PA_ConsultaUsuarios`;
...
(
 ...
 BEGIN
  ...
  IF paEstadoUsuario <> 'null' then 
    SET @QUERY = CONCAT ('SELECT * FROM ( ', @QUERY, ' ) AS T WHERE T.estado IN (',paEstadoUsuario,')');      
  end If; 
